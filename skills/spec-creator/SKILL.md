...
3. **Coletar informações para properties** (perguntar ao usuário):
   ...
   - **Estimativa** — obrigatório. Carregar `ESTIMATION_SCALE` do projeto:
     1. Localizar `.claude/conventions/estimation.md` (raiz; ou `{SUB_PROJECT}/.claude/conventions/estimation.md` se sub-projeto tiver convenção própria em monorepo).
     2. Se ausente: alertar e bloquear (`"O arquivo .claude/conventions/estimation.md nao existe. Rode /update-framework para criar via wizard."`).
     3. Parsear a seção `## Valores válidos`: extrair valores da primeira coluna da tabela, ignorando cabeçalho e linhas com `(exemplo)`/`...`.
     4. Se a lista ficar vazia (só placeholders): alertar `"O arquivo de convencao ainda tem apenas valores de exemplo. Preencha a tabela antes de criar specs."` e bloquear.

     **Independente da Complexidade.** Apresentar `ESTIMATION_SCALE` como opções via AskUserQuestion e **validar** que o valor escolhido pertence à lista. Se `--from` tem story points, sugerir o valor mais próximo da escala — mas confirmar com o usuário antes de aplicar.

     Formato da pergunta: `Estimativa: escolha um da escala do projeto: {ESTIMATION_SCALE}`.
