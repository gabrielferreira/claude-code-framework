...
- [x] task-runner cria branch `task/{spec-id}-{task-index}` antes de cada task
- [x] ao concluir, mostra diff resumido e aguarda confirmação do dev para merge
- [x] se task falhar, instrui o dev a descartar o branch (`git branch -D`) — não tenta recuperar automaticamente
- [x] comportamento é opt-in: task-runner pergunta no início da sessão se quer usar git isolation
- [x] funciona mesmo quando `git worktree` não está disponível (fallback: branch normal sem worktree separado)
