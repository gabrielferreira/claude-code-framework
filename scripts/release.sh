...
echo "  7. Criar GitHub Release com notas do CHANGELOG:"
echo "       notes=\$(awk -v t=\"${NEW_VERSION}\" 'BEGIN{f=0} /^## \\[/{if(f)exit; if(index(\$0,\"[\" t \"]\"))f=1; next} f' CHANGELOG.md)"
echo "       gh release create ${NEW_TAG} --title ${NEW_TAG} --latest --notes \"\$notes\""
echo ""
